...
@section('title', 'Reset Password | E-Library')
...
<div class="min-h-screen flex items-center justify-center px-4 py-10 sm:py-16">
    <div class="w-full max-w-md space-y-8 rounded-[2rem] bg-white p-8 shadow-2xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700">
        <div class="space-y-3 text-center">
            <h1 class="text-3xl font-bold text-slate-950 dark:text-white">Reset kata sandi</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Masukkan password baru Anda untuk menyelesaikan reset.</p>
        </div>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="space-y-6">
            ...
            <div>
                <label for="password" class="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Kata Sandi Baru</label>
            ...
            <button type="submit" class="w-full rounded-3xl bg-violet-600 px-5 py-4 text-base font-semibold text-white shadow-lg shadow-violet-500/20 transition hover:bg-violet-700">Reset Password</button>
        </form>

        <p class="text-center text-sm text-slate-500 dark:text-slate-400">
            <a href="{{ route('login') }}" class="font-semibold text-violet-600 hover:text-violet-700 dark:text-violet-300">Kembali ke Masuk</a>
        </p>
    </div>
</div>
@endsection
